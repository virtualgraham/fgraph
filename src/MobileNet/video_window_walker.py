...
def get_observation_group(tx, id):
    result = tx.run("MATCH (o:Observation) WHERE o.id = $id RETURN o.group10 AS group", id=int(id))
    r = result.single()
    # print(r)
    return r["group"]
...
def insert_observation(tx, id, t, y, x, f):
    ...
    tx.run("Create(o:Observation {id:$id, t:$t, y:$y, x:$x})", id=id, t=t, y=y, x=x)
...
def play_annotated_video():
    ...
    # KNN
    p = hnswlib.Index(space='cosine', dim=1280)
    ...
    while vc.isOpened():
        ...
        windows = np.empty((steps[0]*steps[1], window_size, window_size, 3))
        ...
        for x in range(steps[0]):
            for y in range(steps[1]):
                ...
                c = colors[g]
                cv2.circle(frame, (stride*y+round(window_size/2), stride*x+round(window_size/2)), 6, c, cv2.FILLED)
...
def build_graph():
    ...
    for r in range(runs):
        ...
        for t in range(max_batches):
            ...
            windows = np.empty((frame_batch_size, window_size, window_size, 3))
            positions = []
            ids = []
            batch_frame_count = 0

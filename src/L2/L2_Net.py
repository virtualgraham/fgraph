...
def build_L2_net(net_name):
    ...
    weights = python_net_data['weights']
    fix_weight_format(weights)

    weights_cen = python_net_data['weights_cen']
    fix_weight_format(weights_cen)

    return build_cnn(weights), build_cnn(weights_cen), python_net_data['pix_mean'], python_net_data['pix_mean_cen']

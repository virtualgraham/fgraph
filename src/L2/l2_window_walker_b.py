...
def play_annotated_video():
    ...
    memory_graph = MemoryGraph(index_path="./data/index.bin", node_embeddings_path="./data/node_embeddings_index.bin")
    ...
    while vc.isOpened():
        ...
        en_ids, en_distances = memory_graph.knn_query_node_embeddings(observation_id, k=20)

        # get nodes
        nodes = memory_graph.get_nodes(en_ids[0].tolist())

        for i in range(len(nodes)):
            node = nodes[i]
            en_d = en_distances[0][i]
            print(node)
            print(en_d)
            x = node["x"]
            y = node["y"]
        ...
        cv2.circle(frame, (int(round(random_keypoint[1])), int(round(random_keypoint[0]))), 5, colors[0], cv2.FILLED)

...
class MemoryGraphWalker:
    def __init__(self, memory_graph, knn = 30, accurate_prediction_limit = 10, distance_threshold = 0.1, adjacency_radius = 3, prediction_history_length=7, history_community_matches=1, identical_distance=0.01):
    ...
    def _add_observation(self, file, t, pos, adj, feats, patch, obj, labels, distances, walker_id, community_cache, keep_times=False):
        ...
        if adj and walker_id in self.history and len(neighbor_nodes) > 0:
            ...
            for nn in neighbor_nodes:
                ...
                for h in self.history[walker_id]:
        ...
        if walker_id not in self.history:
            self.history[walker_id] = []

        h = self.history[walker_id]

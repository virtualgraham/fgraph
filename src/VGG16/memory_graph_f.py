...
class MemoryGraphWalker:
    def __init__(self, memory_graph, knn = 10, distance_threshold = 1.0, adjacent_distance = 0.50, identical_distance = 0.15, adjacency_radius = 2):
    ...
    def add_observation(self, t, pos, feats, walker_id, adj=True):
        ...
        if adj:
            ...
            if walker_id in self.last_ids:
                ...
                last_feat = self.last_feats[walker_id]

                if self.memory_graph.space == 'cosine':
                    distance = spatial.distance.cosine(last_feat, feats)
                else:
                    distance = np.linalg.norm(last_feat-feats)

                # if not similar to previous
                if distance > self.identical_distance:
                    self.memory_graph.insert_adjacency(last_id, oid, 0.0)
                    adjacency_added = True
                    print("+adj")
                # else:
                #     print("skipping adding adjacency due to similarity to predecessor")


            # find correct predictions and reinforce with adjacency
            if walker_id in self.predictions and adjacency_added:
                ...
                for pred in predictions:
                    ...
                    if distance <= self.distance_threshold:
                        ...
                        # # if not similar to prediction predecessor
                        # f = pred['props_similar_to_prev']["f"]
                        ...
                        # else:
                        #     print("skipping adding prediction due to similarity to predecessor")
            ...
            for n in range(self.knn):
                ...
                if distance <= self.distance_threshold:
                    ...
                    for n in next_adjacencies:
                        ...
                        # label_props = self.memory_graph.get_observation(label)

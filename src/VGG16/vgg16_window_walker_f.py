...
import random
import math 
from memory_graph_e import MemoryGraph, MemoryGraphWalker
import numpy as np
...
def first_pos(segments):
    print("first_pos")

    rad = 1
    pos = (random.random() * segments.shape[0], random.random() * segments.shape[1])

    while True:

        rad_grid = get_rad_grid(pos, rad, segments.shape)
        ...
        for loc in rad_grid:
            ...
            s = extract_window(segments, point).flatten()
            counter = Counter(s)
            c = counter.most_common(1)[0][1]
            if c < (window_size * window_size * super_pixel_portion):
                return point

        rad += 1


def next_pos_play(segments, pos):
    
    rad_grid = get_rad_grid(pos, 1, segments.shape)
...
def next_pos(segments, pos):
    
    if (pos is not None) and (random.random() > 1.0/walk_length):
    ...
    return first_pos(segments), False
...
def show_patches(path_windows, path_features, path_positions, frame_shape, memory_graph):
    ...
    groups = list(memory_graph.search_group(path_features, 0.1, 2, 2))
...
def play_video():
    ...
    def on_click(event, x, y, flags, param):
        ...
        for i in range(playback_random_walk_length):
            pos = next_pos_play(segments, pos)
            if pos is None:
                break
            path.append(pos)
...
def build_graph():
    ...
    for r in range(runs):
        ...
        for t in range(max_frames):
            ...
            for i in range(walker_count):
                pos[i], adj[i] = next_pos(segments, pos[i])
...
video_file = './media/6790941654883880193.mp4'
